  ...
  return (
    <main className="relative h-dvh w-screen overflow-hidden bg-ink-950">
      ...
      <div className="pointer-events-none absolute inset-0 z-10 flex flex-col">
        ...
        {/* Two rows: title block + icon buttons, then the pills. The pills
            sit outside the title column so on phones they get the full width
            (one row) instead of being squeezed beside the icons and stacking. */}
        {/* The header itself lets pointer events through, so dragging the
            globe works in the gaps between controls; only the buttons and
            pills capture input. */}
        <header className="flex flex-col gap-2.5 p-4 sm:p-6">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 flex-col gap-1">
              <h1 className="truncate font-display text-sm font-semibold uppercase tracking-[0.3em] text-white/80 sm:tracking-[0.4em]">
                QuakeStation
              </h1>
